#line 1 "E:\\work\\GitHub\\Template_For_uCOS-II\\Software\\USBStack\\src\\usbd_hid.c"
...
#line 18 "E:\\work\\GitHub\\Template_For_uCOS-II\\Software\\USBStack\\src\\usbd_hid.c"
 ...
 * Consult your license regarding permissions and restrictions.
V5.04:0576 */
#line 19 "E:\\work\\GitHub\\Template_For_uCOS-II\\Software\\USBStack\\src\\usbd_hid.c"
...
#line 20 "E:\\work\\GitHub\\Template_For_uCOS-II\\Software\\USBStack\\src\\usbd_hid.c"
...
void USBD_HID_EP_INTIN_Event (U32 event) {
  ...
  if ((DataOutSentLen >= DataOutToSendLen) &&
    ...
    ptrDataOut          = 0;
    DataOutSentLen      = 0;
    DataOutToSendLen    = usbd_hid_get_report (0x01, USBD_HID_InReport[0], &USBD_HID_InReport[1], USBD_HID_REQ_EP_INT);
    ...
    if (DataOutAsyncReq) {              /* If asynchronous send requested     */
      ...
      if (usbd_hid_inreport_num <= 1) { /* If only one in report in system    */
        if (DataOutUpdateReqMask) {
          ...
          DataOutSentLen        = 0;
          DataOutToSendLen      = usbd_hid_get_report (0x01, 0, &USBD_HID_InReport[1], USBD_HID_REQ_PERIOD_UPDATE);
          if (DataOutToSendLen) {
        ...
        for (i = USBD_HID_InReport[0]; ; i++) {
          ...
          if (DataOutUpdateReqMask & (1 << i)) {
            ...
            DataOutSentLen      = 0;
            DataOutToSendLen    = usbd_hid_get_report (0x01, i+1, &USBD_HID_InReport[1], USBD_HID_REQ_PERIOD_UPDATE);
            if (DataOutToSendLen) {
              ptrDataOut        = USBD_HID_InReport;
              DataOutToSendLen++;
            }
  ...
  if (DataOutToSendLen ||
    ...
    if ((DataOutSentLen < usbd_hid_inreport_max_sz) &&
      ...
      DataOutEndWithShortPacket = 1;
    } else {
      DataOutEndWithShortPacket = 0;
    }
  }
}
...
void USBD_HID_EP_INTOUT_Event (U32 event) {
  ...
  if (!bytes_rece ||
    ...
    if (usbd_hid_outreport_num <= 1) {  /* If only one out report in system   */
      usbd_hid_set_report (0x02,                    0 ,  USBD_HID_OutReport   , DataInReceLen,   USBD_HID_REQ_EP_INT);
    } else {
      usbd_hid_set_report (0x02, USBD_HID_OutReport[0], &USBD_HID_OutReport[1], DataInReceLen-1, USBD_HID_REQ_EP_INT);
    }
    DataInReceLen = 0;
  }
}
...
void USBD_HID_SOF_Event (void) {
  ...
  if (USBD_Configuration) {
    ...
    if (USBD_HID_PollingCnt == usbd_hid_interval[USBD_HighSpeed]) {
      polling_reload = 1;          /* If polling interval expired        */
    }
    for (i = 0; i < usbd_hid_inreport_num; i++) {
      idle_reload = 0;
      if (tick_4ms) {
        ...
        if (USBD_HID_IdleReload[i]) {
          if (USBD_HID_IdleCnt[i] >= USBD_HID_IdleReload[i]) {
            idle_reload = 1;       /* If idle period expired             */
          }
        }
      }
...
#line 439 "E:\\work\\GitHub\\Template_For_uCOS-II\\Software\\USBStack\\src\\usbd_hid.c"
...
BOOL usbd_hid_get_report_trigger (U8 rid, U8 *buf, int len) {
  ...
  if (USBD_Configuration) {
    DataOutAsyncReq    = 1;        /* Asynchronous data out request      */
    while (DataOutToSendLen) {
      if (!USBD_Configuration) {        /* If device not configured reject rq */
        DataOutAsyncReq    = 0;   /* Asynchronous data out request      */
        ptrDataOut         = 0;
        DataOutSentLen     = 0;
        DataOutToSendLen   = 0;
        return (0);
