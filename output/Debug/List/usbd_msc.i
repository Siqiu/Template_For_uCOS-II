#line 1 "E:\\work\\GitHub\\Template_For_uCOS-II\\Software\\USBStack\\src\\usbd_msc.c"
...
#line 18 "E:\\work\\GitHub\\Template_For_uCOS-II\\Software\\USBStack\\src\\usbd_msc.c"
 ...
 * Consult your license regarding permissions and restrictions.
V5.04:0576 */
#line 19 "E:\\work\\GitHub\\Template_For_uCOS-II\\Software\\USBStack\\src\\usbd_msc.c"
...
#line 20 "E:\\work\\GitHub\\Template_For_uCOS-II\\Software\\USBStack\\src\\usbd_msc.c"

BOOL        USBD_MSC_MediaReady   = 0;
BOOL        USBD_MSC_ReadOnly     = 0;
...
BOOL        USBD_MSC_MediaReadyEx = 0; /* Previous state of Media ready */
...
void USBD_MSC_ClrStallEP (U32 EPNum) {     /* clear EP halt status according stall status */
  ...
  if ((n == (usbd_msc_ep_bulkin | 0x80)) && ((USBD_EndPointHalt & m) != 0)) {
    ...
    if (USBD_MSC_CSW.dSignature == 0x53425355) {
...
void USBD_MSC_MemoryVerify (void) {
  ...
  if ((Length == 0) || (BulkStage == 5)) {
    USBD_MSC_CSW.bStatus = (MemOK) ? 0x00 : 0x01;
    USBD_MSC_SetCSW();
  }
}
...
BOOL USBD_MSC_RWSetup (void) {
  ...
  switch (USBD_MSC_CBW.CB[0]) {
    case 0x2A:
    case 0x2F:
      if (!USBD_MSC_CheckMedia()) return (0);
    case 0x28:
...
void USBD_MSC_DataInTransfer (void) {
  ...
  if (BulkLen >= USBD_MSC_CBW.dDataLength) {
    ...
    BulkStage = 4; /* short or zero packet */
  }
...
void USBD_MSC_StartStopUnit (void) {
  ...
  if (!USBD_MSC_CBW.CB[3]) {               /* If power condition modifier is 0 */
    ...
    USBD_MSC_CSW.bStatus = 0x00; /* Start Stop Unit -> pass */
    USBD_MSC_SetCSW();
    return;
  }

  USBD_MSC_CSW.bStatus = 0x01;   /* Start Stop Unit -> fail */
  USBD_MSC_SetCSW();
}
...
void USBD_MSC_MediaRemoval (void) {
  ...
  if (USBD_MSC_CBW.CB[4] & 1)              /* If prevent */
    USBD_MSC_CSW.bStatus = 0x01; /* Prevent media removal -> fail */
  else                                     /* If allow */
    USBD_MSC_CSW.bStatus = 0x00; /* Allow media removal -> pass */
...
void USBD_MSC_GetCBW (void) {
  ...
  if ((BulkLen == sizeof(USBD_MSC_CBW)) && (USBD_MSC_CBW.dSignature == 0x43425355)) {
    /* Valid USBD_MSC_CBW */
...
void USBD_MSC_SetCSW (void) {
  ...
  USBD_MSC_CSW.dSignature = 0x53425355;
  USBD_WriteEP(usbd_msc_ep_bulkin | 0x80, (U8 *)&USBD_MSC_CSW, sizeof(USBD_MSC_CSW));
  BulkStage = 5;
}
...
{
    if (event & (1 << 2)) USBD_MSC_EP_BULKOUT_Event (0);
    if (event & (1 << 3)) USBD_MSC_EP_BULKIN_Event (0);
}
